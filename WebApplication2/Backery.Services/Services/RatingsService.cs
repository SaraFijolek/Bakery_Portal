    ...
    public class RatingsService : IRatingsService
    {
        ...
        public async Task<RatingService<RatingDto>> CreateRatingAsync(CreateRatingDto createRatingDto)
        {
            try
            {
                ...
                var fromUserExists = await _context.Users.AnyAsync(u => u.UserId == createRatingDto.FromUserId);
                var toUserExists = await _context.Users.AnyAsync(u => u.UserId == createRatingDto.ToUserId);
        ...
        public async Task<RatingService<RatingDto>> UpdateRatingAsync(int id, UpdateRatingDto updateRatingDto)
        {
            try
            {
                ...
                // Validate that FromUserId and ToUserId exist
                var fromUserExists = await _context.Users.AnyAsync(u => u.UserId == updateRatingDto.FromUserId);
                var toUserExists = await _context.Users.AnyAsync(u => u.UserId == updateRatingDto.ToUserId);
